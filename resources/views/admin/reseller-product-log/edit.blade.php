...
@section('title', 'Edit Reseller Log')

@section('content')
<section class="section">
    <div class="section-header">
        <h1>Edit Reseller Log</h1>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
        <form method="POST" action="{{ route('admin.reseller-product-logs.update', $resellerProductLog) }}">
            @csrf
            @method('PUT')
                ...
                <div class="col-sm-12 col-md-7">
                    <select name="product_id" class="form-control" required>
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ $resellerProductLog->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                        @endforeach
                    </select>
                    @error('product_id')
            ...
            <div class="form-group row mb-4">
                ...
                <div class="col-sm-12 col-md-7">
                    <textarea name="log_item" class="form-control summernote-log-item" rows="6" required>{{ $resellerProductLog->log_item }}</textarea>
                    @error('log_item')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group row mb-4">
                ...
                <div class="col-sm-12 col-md-7">
                    <textarea name="details" class="form-control summernote" rows="3">{{ $resellerProductLog->details }}</textarea>
                    @error('details')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group row mb-4">
                ...
                <div class="col-sm-12 col-md-7">
                    <select name="status" class="form-control" required>
                        <option value="available" {{ $resellerProductLog->status == 'available' ? 'selected' : '' }}>Available</option>
                        <option value="sold" {{ $resellerProductLog->status == 'sold' ? 'selected' : '' }}>Sold</option>
                    </select>
                    @error('status')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
